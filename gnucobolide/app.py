...
from gnucobolide import __version__, logger, system
from gnucobolide.controllers import (
    CobolController, EditController, FileController, HelpController,
    HomeController, ViewController)
from gnucobolide.controllers.cobol import CompilationThread
from gnucobolide.compilers import check_compiler, CompilerNotFound, \
    GnuCobolCompiler
from gnucobolide.settings import Settings
from gnucobolide.view.main_window import MainWindow
from gnucobolide.view.dialogs.about import DlgAbout
...
class Application(QtCore.QObject):
    ...
    def handle_command_line_args(self):
        ...
        # show runtime env
        if args.runtime_env:
            print('GNUCobolIDE {}'.format(__version__))
            for k, v in sorted(DlgAbout.get_runtime_env().items(), key=lambda x: x[0]):
                print('{} {}' % (k, v))
            sys.exit(0)
